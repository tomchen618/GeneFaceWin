...
class RADNeRFDataset(torch.utils.data.Dataset):
    def __init__(self, prefix, data_dir=None, training=True):
        ...
        if self.cond_type == 'deepspeech':
            ...
            # this two code lines is switched for the warning. Tom Chen 2024-2-24
            self.lm68s = self.lm2ds[:, index_lm68_from_lm478, :].clone().detach()
    ...
    def __getitem__(self, idx):
        ...
        sample['lm68'] = self.lm68s[idx].reshape([68 * 2]).clone().detach()
        if self.training:
