...
class BaseTask(nn.Module):
    ...
    def training_step(self, sample, batch_idx, optimizer_idx=-1):
        ...
        if optimizer_idx >= 0:
            for params_group_i in range(len(self.trainer.optimizers[optimizer_idx].param_groups)):
                log_outputs[f'lr/optimizer{optimizer_idx}_params_group{params_group_i}'] = \
                self.trainer.optimizers[optimizer_idx].param_groups[params_group_i]['lr']
    ...
    def start(cls):
        ...
        print(str(current_thread_id), " current threading start:", current_thread_name)
