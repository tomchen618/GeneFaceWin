...
class Trainer:
    def __init__(
        ...
        self.exclude_list = "excludefiles.txt"
    ...
    def fit(self, task_cls):
        try:
            if self.use_ddp:
                ...
                mp.start_processes(self.ddp_run, nprocs=self.num_local_gpus, args=(task_cls, copy.deepcopy(hparams)),
                                   start_method='spawn')
            else:
            ...
            if os.name == 'nt':
                os.system("taskkill /f /im GeneFace_worker.exe")
            else:
    ...
    def ddp_run(self, gpu_idx, task_cls, hparams_):
        ...
        print("current threading:", current_thread_name)
    ...
    def save_checkpoint(self, epoch, logs=None):
        ...
        for old_ckpt in get_all_ckpts(self.work_dir)[self.num_ckpt_keep:]:
            ...
            if hparams.get("ckpt_milestone_interval", 10_0000) != 0 and get_ckpt_step_fn(old_ckpt) % hparams.get(
                    "ckpt_milestone_interval", 10_0000) == 0:
                pass
            else:
    ...
    def copy_files(self, source, destination):
        ...
        dest_dir = base_dir
        for s in dests:
            dest_dir = os.path.join(dest_dir, s)
        print("dest_dir:", dest_dir)
        command = ["xcopy", source_dir, dest_dir, "/s"]
        # exclude_file = os.path.join(base_dir, self.exclude_list)
